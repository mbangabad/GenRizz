...
export default function LeaderboardEntry({ score, rank, isMe, showGame = true, showChallenge = false, gameId }) {
  ...
  return (
    <motion.div 
      ...
      className={`card-3d p-4 ${isMe ? 'ring-2 ring-[#CE82FF] card-3d-purple' : ''}`}
    >
      <div className="flex items-center gap-3">
        {/* Rank */}
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0 ${
          rank === 1 ? 'bg-gradient-to-br from-[#FFC800] to-[#FFB347] text-white' :
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="font-bold text-[#3C3C3C] truncate">
              {isMe ? 'You' : (score.user_name || `Player ${score.user_id?.slice(-4)}`)}
            </p>
            {isPremium && (
              <div className="w-5 h-5 rounded bg-gradient-to-br from-[#FFC800] to-[#FFB347] flex items-center justify-center flex-shrink-0"
          ...
          {showGame && game && (
            <p className="text-xs text-[#777777] font-semibold">{game.icon} {game.title}</p>
          )}
          {score.tier_name && (
            <p className="text-xs text-[#AFAFAF] font-semibold">{score.tier_emoji} {score.tier_name}</p>
          )}
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-xl font-black text-[#3C3C3C]">{score.percentage}%</p>
            <p className="text-xs text-[#AFAFAF] font-semibold">{score.score}/{score.max_score}</p>
          </div>
          
          {showChallenge && !isMe && (
            <Link 
              to={createPageUrl(`Challenges?challengedId=${score.user_id}&gameId=${gameId || score.game_id}`)}
              className="p-2.5 rounded-xl bg-[#FF9600]/20 text-[#FF9600] hover:bg-[#FF9600]/30 transition-colors"
              title="Challenge"
            >
