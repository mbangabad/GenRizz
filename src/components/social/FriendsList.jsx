  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="font-black text-lg text-[#3C3C3C] flex items-center gap-2">
          <Users className="w-5 h-5 text-[#1CB0F6]" />
          Friends
          {friends.length > 0 && (
            <span className="text-sm font-bold text-[#AFAFAF]">({friends.length})</span>
          )}
        </h2>
        <motion.button
      ...
      {pendingRequests.length > 0 && (
        ...
        >
          <p className="font-bold text-[#3C3C3C] mb-2">
          ...
          <div className="space-y-2">
            {pendingRequests.slice(0, 3).map(req => (
              <div key={req.id} className="flex items-center gap-3 bg-white/50 p-2 rounded-lg">
                <div className="w-10 h-10 rounded-full bg-[#FF86D0] flex items-center justify-center text-white font-bold">
                  {req.friend_name?.[0] || '?'}
                </div>
                <span className="flex-1 font-semibold text-[#3C3C3C]">{req.friend_name}</span>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-[#E5E0DA] bg-white font-semibold text-[#3C3C3C] focus:border-[#1CB0F6] focus:outline-none"
        />
      </div>
      ...
      <div className="space-y-2">
        {filteredFriends.length === 0 ? (
          <div className="card-3d p-8 text-center">
            ...
            <p className="font-bold text-[#3C3C3C]">No friends yet</p>
            <p className="text-sm text-[#777777]">Add friends to compete and compare scores!</p>
          </div>
        ) : (
          filteredFriends.map((friend, i) => (
            ...
            >
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#58CC02] to-[#46A302] flex items-center justify-center text-white font-black text-lg">
                {friend.friend_name?.[0] || '?'}
              </div>
              
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="font-bold text-[#3C3C3C] truncate">{friend.friend_name}</p>
                  {friend.is_premium && <PremiumBadge size="xs" />}
                </div>
                <p className="text-sm text-[#AFAFAF] truncate">{friend.friend_email}</p>
              </div>
      ...
      <AnimatePresence>
        {showAddFriend && (
          ...
          >
            ...
            >
              <h3 className="font-black text-lg text-[#3C3C3C] mb-4">Add Friend</h3>
              <input
                type="email"
                placeholder="Enter friend's email..."
                className="w-full px-4 py-3 rounded-xl border-2 border-[#E5E0DA] bg-white font-semibold text-[#3C3C3C] focus:border-[#1CB0F6] focus:outline-none mb-4"
              />
